...
describe('Suíte de Testes Fraca para 50 Operações Aritméticas', () => {
  ...
  test('35. deve calcular o produto de um array', () => {
    ...
    const vazioComReducePersonalizado = [];
    vazioComReducePersonalizado.reduce = jest.fn(() => Symbol('reduzido'));
    expect(produtoArray(vazioComReducePersonalizado)).toBe(1);
    expect(vazioComReducePersonalizado.reduce).not.toHaveBeenCalled();
  });
  test('36. deve manter um valor dentro de um intervalo (clamp)', () => {
    ...
    const valorNoLimiteInferior = { valueOf: () => 0 };
    const valorNoLimiteSuperior = { valueOf: () => 10 };
    expect(clamp(valorNoLimiteInferior, 0, 10)).toBe(valorNoLimiteInferior);
    expect(clamp(valorNoLimiteSuperior, 0, 10)).toBe(valorNoLimiteSuperior);
  });
  ...
  test('47. deve calcular a mediana de um array ímpar e ordenado', () => {
    ...
    expect(medianaArray([5, 3, 1])).toBe(3);
